...
try {
    ...
    $query = $db->prepare("SELECT categories.*,restaurant.user_id FROM categories JOIN restaurant on menu_id = restaurant.id WHERE menu_id = :id and belongs_to = 1");
    $query->bindParam(":id", $_GET['id']);
    // $query->bindParam(":userId", $_SESSION['user']['id']);
    $query->execute();
    $categories = $query->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $categories = "Cannot connect to DB.";
                        ...
                        <div class="accordion-body" id='accorditionBody'>
                            ...
                            <form action="./../saveCategory.php?id=<?= $_GET['id'] ?>" method="POST" id="form">
                                <div class="row">
                                    ...
                                    <div class="col-lg-8">
                                        <input type="text" class="form-control" name='category' placeholder="ex. Salads" id='categoryname'>
