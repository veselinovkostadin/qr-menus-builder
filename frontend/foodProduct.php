...
$query = $db->prepare("SELECT products.*,restaurant.name as restaurant,categories.name as category FROM products JOIN categories ON category_id = categories.id
               JOIN restaurant ON categories.menu_id = restaurant.id WHERE products.id = :id and restaurant.UUID = :uuid");
$query->execute([":uuid" => $_GET["UUID"], ":id" => $_GET['id']]);
...
<head>
     ...
     <link rel="stylesheet" href="./styles/foodProduct.css">
...
<body>
     <nav class="navbar bg-transparent">
          <div class="container-fluid">
               <a id="logoname" href="#" class="col-8"><?= $product['restaurant'] ?></a>
     ...
     <div class="container my-4 d-flex justify-content-center align-items-center">
          <div class="row">
               <div class="col-12">
                    <img src="<?= generateProductImage($product['picture']) ?>" width="250px" height="250px" class="rounded-circle" alt="product image">
               </div>
          </div>
     </div>
     <div class="container">
     ...
     <div class="container mb-4">
          <div class="row d-flex justify-content-center text-center">
               <div class="col-10 prodDescription">
                    <p class="pt-3 px-2"><?= $product['description'] ?></p>
     ...
     <div class="container d-flex justify-content-around sticky-bottom my-2">
          <div class="col-5 drinkBtn">
               <a href="./drinkSection.php?UUID=<?= $_GET['UUID'] ?>"><span>Drink</span></a>
          </div>
          <div class="col-5 backBtn">
               <a href="./foodCategory.php?UUID=<?= $_GET["UUID"] ?>&categoryId=<?= $product['category_id'] ?>"><span>Back</span></a>
          </div>
     </div>
