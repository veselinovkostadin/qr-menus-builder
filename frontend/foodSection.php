...
$query = $db->prepare("SELECT categories.*,restaurant.name as restaurant FROM categories JOIN restaurant ON categories.menu_id = restaurant.id WHERE restaurant.UUID = :uuid and categories.belongs_to = 1");
$query->bindParam(":uuid", $_GET["UUID"]);
$query->execute();
...
<body>
     ...
     <div class="container mt-4" id="categories">
          ...
          if (!empty($categories)) {
               foreach ($categories as $category) {
                    echo "<div class='foodCategories leftShadow row mx-auto mb-2'>
               <a href='./foodCategory.php?UUID={$_GET['UUID']}&categoryId={$category['id']}' class='d-flex justify-content-center align-items-center mb-0'>{$category['name']}</a>
          </div>";
               }
          } else {
               echo "<div class='foodCategories leftShadow row mx-auto mb-2'>
     ...
     <div class="container d-flex justify-content-end my-2 fixed-bottom">
